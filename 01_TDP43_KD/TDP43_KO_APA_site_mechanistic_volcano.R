...
TABLE_OUT <- file.path(
  OUT_DIR,
  "TDP43_KO_APA_site_mechanistic_results.tsv"
)

PDF_OUT <- file.path(
  OUT_DIR,
  "TDP43_KO_APA_site_mechanistic_volcano.pdf"
)

PNG_OUT <- file.path(
  OUT_DIR,
  "TDP43_KO_APA_site_mechanistic_volcano.png"
)
...
dat$Control_mean_PAU <- rowMeans(
  dat[, WT_PAU, drop = FALSE],
  na.rm = TRUE
)

dat$TDP43_KO_mean_PAU <- rowMeans(
...
#
# positive:
# increased usage in TDP-43 KO
#
# negative:
# decreased usage in TDP-43 KO
# ============================================================

dat$DeltaPAU <- (
  dat$TDP43_KO_mean_PAU -
  dat$Control_mean_PAU
)
...
p <- ggplot(
  ...
  labs(
    ...
    title =
      "TDP-43 KO alters alternative polyadenylation",
