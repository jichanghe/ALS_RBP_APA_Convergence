...
BASE_DIR <- "results/04_FUS_P525L"
INPUT <- file.path(BASE_DIR, "FUS_P525L_QAPA_PAU.tsv")

XLSX_OUT <- file.path(BASE_DIR, "FUS_P525L_differential_APA.xlsx")
VOLCANO_OUT <- file.path(BASE_DIR, "FUS_P525L_volcano.pdf")
PCA_OUT <- file.path(BASE_DIR, "FUS_P525L_PCA.pdf")
HEATMAP_OUT <- file.path(BASE_DIR, "FUS_P525L_heatmap.pdf")
SL_OUT <- file.path(BASE_DIR, "FUS_P525L_shortening_lengthening.pdf")
MECH_VOLCANO_OUT <- file.path(BASE_DIR, "FUS_P525L_APA_site_mechanistic_volcano.pdf")

GO_DIR <- file.path(BASE_DIR, "GO_analysis")
dir.create(GO_DIR, recursive = TRUE, showWarnings = FALSE)
GO_XLSX <- file.path(GO_DIR, "FUS_P525L_GO_enrichment.xlsx")
GO_GENES_XLSX <- file.path(GO_DIR, "FUS_P525L_GO_gene_lists.xlsx")
...
control_pau <- c("WT_Rep1.PAU", "WT_Rep2.PAU", "WT_Rep3.PAU")
case_pau <- c("P525L_Rep1.PAU", "P525L_Rep2.PAU", "P525L_Rep3.PAU")
control_tpm <- c("WT_Rep1.TPM", "WT_Rep2.TPM", "WT_Rep3.TPM")
case_tpm <- c("P525L_Rep1.TPM", "P525L_Rep2.TPM", "P525L_Rep3.TPM")
...
p_volcano <- ggplot(
  ...
  labs(
    title = "FUS P525L differential alternative polyadenylation",
    subtitle = paste0(
      "Nominal P < ",
      NOMINAL_P_CUTOFF,
      " and |DeltaPAU| >= ",
...
pca_plot_df$Group <- ifelse(pca_plot_df$Sample %in% control_pau,"Control","FUS P525L")
...
annotation_col <- data.frame(Group=c(rep("Control",length(control_pau)),rep("FUS P525L",length(case_pau))))
...
p_sl <- ggplot(
  ...
  # ----------------------------------------------------------
  labs(
    title = "FUS P525L alters 3'UTR length",
    subtitle = "Gene-level PAU-weighted 3'UTR usage",
    x = expression(
      Delta*"3'UTR length ("*"FUS P525L"*" - Control, kb)"
    ),
    y = expression(
      -log[10]*"(P value)"
    )
...
p_mech <- ggplot(mech,aes(DeltaPAU,minus_log10_P)) +
  ...
  labs(title="FUS P525L: mechanistic APA-site changes",
       subtitle="Blue = 3'UTR shortening; red = 3'UTR lengthening",
       x="DeltaPAU (FUS P525L - Control)",y="-log10(P value)") +
  theme_classic(base_size=13)
...
plot_GO <- function(dat,direction,ontology,outfile,top_n=TOP_GO) {
  ...
  p <- ggplot(x,aes(GeneRatio,GO_term)) +
    ...
    labs(title=paste0("GO ",ontology_text," associated with FUS P525L 3'UTR ",tolower(direction)," genes"),
...
plot_GO(short_result,"Shortening","BP",file.path(GO_DIR,"FUS_P525L_shortening_GO_BP.pdf"))
plot_GO(short_result,"Shortening","CC",file.path(GO_DIR,"FUS_P525L_shortening_GO_CC.pdf"))
plot_GO(short_result,"Shortening","MF",file.path(GO_DIR,"FUS_P525L_shortening_GO_MF.pdf"))
plot_GO(long_result,"Lengthening","BP",file.path(GO_DIR,"FUS_P525L_lengthening_GO_BP.pdf"))
plot_GO(long_result,"Lengthening","CC",file.path(GO_DIR,"FUS_P525L_lengthening_GO_CC.pdf"))
plot_GO(long_result,"Lengthening","MF",file.path(GO_DIR,"FUS_P525L_lengthening_GO_MF.pdf"))

message("==============================================")
message("ANALYSIS COMPLETE: FUS P525L")
message("==============================================")
print(summary_table)
